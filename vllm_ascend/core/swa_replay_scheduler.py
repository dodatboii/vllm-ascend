...
"""SWA bounded replay: replay-aware scheduler (prefix rewind).

A sliding-window group that declares ``bounded_replay`` opts out of prefix
caching (``prefix_cacheable=False``) and is rebuilt after a prefix hit by
recomputing the hit's trailing window, so its KV never has to be persisted.
The hit's blocks stay shared and are not rewritten; only the global
(prefix-cacheable) groups are reused verbatim.
"""
...
except ImportError:  # vLLM 0.28.0 release lane
    from vllm.v1.core.sched.output import (  # type: ignore[assignment]
        NewRequestData,
        SchedulerOutput,
    )
...
class SwaReplayScheduler(Scheduler):
    """Scheduler that rewinds a prefix hit by the replayed window.

    Behaves exactly like the upstream scheduler when no KV cache group
    declares a replay window (``prefix_replay_tokens == 0``).
    """
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if request.num_computed_tokens == 0:
                    ...
                    if self.connector is not None:
                        ...
                        # [SWA-REPLAY H2] A connector hit is taken in whole
                        # blocks: a hit ending one token short of a block
                        # boundary would retire the sliding-window block the
                        # replay starts in.
                    ...
                    # [SWA-REPLAY H3] A hit no longer than the replayed window
                    # would be recomputed in full and save nothing; drop it
                    # (an async load of it is not even started).
                ...
                # [SWA-REPLAY H4] Recompute the tail of the hit without
                ...
                if load_kv_async:
                    ...
                    # [SWA-REPLAY H4] Fresh admission: local and/or sync
                    # external hit. Rewind the computed count over the
                    # replayed tokens so this step recomputes them.
                ...
                # [SWA-REPLAY H5] Replayed tokens are already counted in the
                # adopted hit; a chunk that ends inside the replayed range
                # needs no new slots.
        ...
        with record_function_or_nullcontext("schedule: make_cached_request_data"):
            ...
            # [SWA-REPLAY H7] Ascend V1 only, beyond #56227: a preempted
            # request resumes through CachedRequestData (V2 folds resumed
            # requests into the new-request list), so its replay start has
            # to travel with it. Empty unless a resumed request replays.
            cached_reqs_data.replay_start = {
                req.request_id: req.replay_start
    ...
    def _update_waiting_for_remote_kv(self, request: Request) -> None:
        ...
        # [SWA-REPLAY H6] SWA bounded replay recomputes the tail of the hit,
        # which covers the last token; otherwise a full prompt hit
        # re-computes that token so the next one can be sampled.
        num_replay_tokens = self._mark_prefix_replay(
...
class AsyncSwaReplayScheduler(AsyncScheduler, SwaReplayScheduler):
    pass
